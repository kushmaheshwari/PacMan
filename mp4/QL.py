...
class QL:
	...
	def updateQMatrix(self, ball_x, ball_y, velocity_x, velocity_y, paddle_y, possibleAction, reward):
		...
		if (pos[0] > 11) and (pos[1] != paddle_y):
			...
			self.NMatrix[ball_x, ball_y, velocity_x, velocity_y+1, paddle_y, possibleAction] = 1 + self.NMatrix[ball_x, ball_y, velocity_x, velocity_y+1, paddle_y, possibleAction] 
			print(np.argmax(self.NMatrix[ball_x, ball_y, velocity_x, velocity_y+1, paddle_y, possibleAction]))
			...
			self.NMatrix[ball_x, ball_y, velocity_x, velocity_y+1, paddle_y, possibleAction] = 1 + self.NMatrix[ball_x, ball_y, velocity_x, velocity_y+1, paddle_y, possibleAction]
			print(np.argmax(self.NMatrix[ball_x, ball_y, velocity_x, velocity_y+1, paddle_y]))
	...
	def bestAction(self, ball_x, ball_y, velocity_x, velocity_y, paddle_y):
		...
		print('Threshold :' + str(50//(np.argmax(self.NMatrix[ball_x, ball_y, velocity_x, velocity_y+1, paddle_y])+1)))
		if randomInt < 50//(np.argmax(self.NMatrix[ball_x, ball_y, velocity_x, velocity_y+1, paddle_y])+1):
